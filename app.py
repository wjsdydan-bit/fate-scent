...
def load_data(path: str):
    ...
    # 텍스트 필드 정리
    for c in ["matched_keywords", "all_text", "Notes", "Description", "Brand", "Name", "Image URL"]:
    ...
    mask = ~df["Name"].astype(str).str.lower().apply(lambda x: any(w in x for w in exclude_words))
    df = df[mask].copy()
...
def dynamic_story_kr(name: str, season: str, strongest: str, weakest: str):
    se = season_ko(season)
    return f"""
**{name}**님은 **{se}의 기운**이 바탕에 깔려 있어요.  
지금 계산된 오행 균형에서는 **{strongest} 기운이 상대적으로 강하고**, **{weakest} 기운이 부족**해요.  
👉 그래서 오늘 추천은 **`{weakest}`를 보완해주는 향수** 중심으로 골랐습니다.
"""
...
ELEMENT_KEYWORDS = {
    ...
    "Metal": ["aldehyde", "metallic", "mineral", "cool", "mint", "soapy", "clean", "white floral"],
...
KW_KO = {
    ...
    "mint":"민트","mineral":"미네랄","metallic":"메탈릭","cool":"쿨"
}
